...
type FileSystemEntryLike = {
  ...
  name?: string;
  createReader?: () => {
    readEntries: (callback: (entries: FileSystemEntryLike[]) => void, errorCallback?: (error: DOMException) => void) => void;
  };
...
async function readEntry(entry: FileSystemEntryLike): Promise<UploadItem[]> {
  if (entry.isFile && entry.file) {
    ...
    return new Promise((resolve) => {
      readFile((file) => {
        resolve([{ file, path: normalizeUploadPath(entry.fullPath || entry.name || file.name) }]);
      });
    });
  }
  ...
  return (
    <section className="overflow-hidden rounded-md border border-[#30363d] bg-[#0d1117] text-[#f0f6fc]">
      ...
      >
        ...
        <button
          className="mt-3 inline-flex h-10 items-center justify-center gap-2 rounded-md border border-[#30363d] bg-[#21262d] px-4 text-sm font-medium text-[#f0f6fc] transition hover:-translate-y-0.5 hover:bg-[#30363d]"
        ...
        <input
          ...
          ref={folderInputRef}
          type="file"
          // @ts-expect-error React does not type Chromium's folder picker attributes.
          webkitdirectory=""
        />
      </div>
