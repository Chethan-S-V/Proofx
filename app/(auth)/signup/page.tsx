...
import { signupWithEmail, signInWithOAuth } from "../../../src/lib/auth/service";
...
export default async function SignupPage({ searchParams }: SignupPageProps) {
  ...
  async function oauthSignup(formData: FormData) {
    "use server";
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f6f7f9] text-slate-950">
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-7xl items-center justify-center px-5 py-8 sm:px-8 lg:px-12">
        <div className="relative grid w-full grid-cols-1 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.12)] lg:grid-cols-[0.9fr_1.1fr]">
          {/* Left Side - OAuth Providers */}
          <div className="animate-slide-in-left flex flex-col justify-between bg-slate-950 p-6 text-white sm:p-10 lg:min-h-176 lg:p-12">
            <div>
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium text-slate-400">Continue with</p>
              <div className="grid grid-cols-3 gap-2">
              
              {/* Google Button */}
              <form action={oauthSignup}>
                <input type="hidden" name="provider" value="google" />
                <button
                  type="submit"
                  className="oauth-btn flex h-9 w-full items-center justify-center gap-2 rounded-md border border-white/10 bg-white/4 px-3 text-xs font-semibold text-white outline-none hover:border-white/20 focus:border-cyan-300/60 focus:ring-4 focus:ring-cyan-300/10"
                >
            ...
            <div className="mt-10 grid gap-3 text-sm text-slate-300">
              <div className="animate-fade-in-up flex items-center gap-3 rounded-lg border border-white/10 bg-white/4 px-4 py-3" style={{ animationDelay: "0.2s" }}>
                <div className="h-2 w-2 shrink-0 rounded-full bg-cyan-300"></div>
                <p className="text-sm text-slate-300">Secure private account verification</p>
              </div>
              ...
              <div className="animate-fade-in-up flex items-center gap-3 rounded-lg border border-white/10 bg-white/4 px-4 py-3" style={{ animationDelay: "0.4s" }}>
                <div className="h-2 w-2 shrink-0 rounded-full bg-cyan-300"></div>
                <p className="text-sm text-slate-300">Share trusted credentials</p>
              </div>
            </div>
          </div>

          {/* Right Side - Form */}
          <div className="animate-slide-in-right flex items-center justify-center p-6 sm:p-10 lg:p-12">
            <div className="w-full max-w-136">
              <div className="mb-8">
                <p className="text-sm font-medium text-slate-500">Start your ProofX account</p>
          ...
          <Link aria-label="Open Wild En Tree" className="absolute bottom-0 left-1/2 z-20 h-16 w-36 -translate-x-1/2 lg:left-[45%]" href="/wet" rel="noreferrer" target="_blank">
